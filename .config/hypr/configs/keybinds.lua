...
hl.bind('CTRL + SHIFT + ESCAPE', hl.dsp.exec_cmd(terminal .. ' --title=btop --app-id=btop btop'), { description = 'open system monitor', submap_universal = true})
...
hl.bind('ALT + X',      hl.dsp.exec_cmd('pkill rofi || cliphist list | rofi -dmenu -p clipboard | cliphist decode | wl-copy'), { release = true })
...
-- Relative & special
hl.bind(mod .. ' + CTRL + L', hl.dsp.focus({ workspace = 'r+1' }), { release = true })
hl.bind(mod .. ' + CTRL + H', hl.dsp.focus({ workspace = 'r-1' }), { release = true })
hl.bind(mod .. ' + CTRL + J', hl.dsp.focus({ workspace = 'empty' }),{ release = true })
hl.bind(mod .. ' + CTRL + K', hl.dsp.focus({ workspace = 'previous' }), { release = true })
...
-- hl.bind('SHIFT + CTRL', hl.dsp.group.next(),{ release = true })
...
for _, i in ipairs(move_directions) do
	...
	-- hl.bind(mod .. ' + SHIFT + ' .. i.key, hl.dsp.window.move({ x = i.x, y = i.y, repeating = true }))
...
local z = {
  { key = 'COMMA', tiling = 'left' },
  { key = 'PERIOD', tiling = 'right' }
}

for _, i in ipairs(z) do
  hl.bind(mod  .. ' + ' .. i.key, hl.dsp.focus({ direction = i.tiling }), { release = true })
end
...
hl.bind(mod .. ' + ALT + RIGHT', hl.dsp.exec_cmd('wpctl set-volume -l 1 @DEFAULT_AUDIO_SINK@ 5%+'), { locked = true, repeating = true })
hl.bind('XF86AudioLowerVolume',      hl.dsp.exec_cmd('wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%-'),      { locked = true, repeating = true })
hl.bind(mod .. ' + ALT + LEFT', hl.dsp.exec_cmd('wpctl set-volume @DEFAULT_AUDIO_SINK@ 5%-'),      { locked = true, repeating = true })
hl.bind('XF86AudioMute',             hl.dsp.exec_cmd('wpctl set-mute @DEFAULT_AUDIO_SINK@ toggle'),     { locked = true, repeating = true })
hl.bind('XF86AudioMicMute',          hl.dsp.exec_cmd('wpctl set-mute @DEFAULT_AUDIO_SOURCE@ toggle'),   { locked = true, repeating = true })

hl.bind('XF86MonBrightnessUp',       hl.dsp.exec_cmd('brightnessctl -e4 -n2 set 5%+'), { locked = true, repeating = true })
hl.bind(mod .. ' + ALT + UP',        hl.dsp.exec_cmd('brightnessctl -e4 -n2 set 5%+'), { locked = true, repeating = true })
hl.bind('XF86MonBrightnessDown',     hl.dsp.exec_cmd('brightnessctl -e4 -n2 set 5%-'), { locked = true, repeating = true })
hl.bind(mod .. ' + ALT + DOWN',        hl.dsp.exec_cmd('brightnessctl -e4 -n2 set 5%-'), { locked = true, repeating = true })
...
hl.bind('ALT + ESCAPE', hl.dsp.exec_cmd('pidof waybar && pkill waybar || waybar'), { description = 'toggle waybar' , release = true })
...
hl.bind(mod .. ' + M', hl.dsp.exec_cmd(scripts .. 'wallpaper/wallpaper.sh -s'), { description = 'gui wallpaper selector', release = true })
hl.bind(mod .. ' + N', hl.dsp.exec_cmd(scripts .. 'wallpaper/wallpaper.sh -r'), { description = 'random wallpaper toggle', release = true })
hl.bind(mod .. ' + L', hl.dsp.exec_cmd(scripts .. 'wallpaper/live.sh -r'), { description = 'gui video wallpaper', release = true })
...
--  hl.window_rule({ match = { class = '.*' }, opacity = 1.0 })
--end)
