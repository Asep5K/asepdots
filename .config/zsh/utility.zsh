...
function cd(){
	builtin cd "$@" && command exa --icons=always --group-directories-first --color=auto -F
}

function git-sparse(){
    if [[ -z "$1" || -z "$2" ]]; then
        echo "Usage: git-sparse <repository> <subdirectory>"
        echo "Example: git-sparse https://github.com/user/repo.git src/utils"
        return 1
    fi
    # 
    echo "Sparse cloning... 🚀"
    ...
    # 
    # Clone cuma struktur doang, tanpa file
    git clone --filter=blob:none --sparse "$repo" "$dir_name" 2>/dev/null
    # 
    cd "$dir_name"
    # 
    # Specify folder yang mau di-download
    git sparse-checkout set "$subdir"
    # 
    # Pull actual files-nya
    git pull origin main 2>/dev/null || git pull origin master 2>/dev/null
    # 
    echo "✅ Done! Check folder: $dir_name/$subdir"
    cd ..
}
